...
@implementation ZPMDebugOverlay
...

{
    if(self = [super initWithFrame:frame])
    {
        ...
        [_mainImageButton addTarget:self action:@selector(oneClick:) forControlEvents:UIControlEventTouchUpInside];
        if (_animationColor) {
        ...
        [self doBorderWidth:myBorderWidth color:[UIColor lightGrayColor] cornerRadius:_frameWidth/2];
...
//点击事件
- (void)oneClick:(UITapGestureRecognizer*)p
{
    [self stopAnimation];
    ...
    if (!self.isShowTab) {
        ...
        [UIView animateWithDuration:showDuration animations:^{
            ...
            if (self.frame.origin.x <= kScreenWidth/2) {
                ...
                self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, WIDTH + self->_imagesAndTitle.count * (self.frameWidth + marginWith) ,self.frameWidth);
            }else{
                
                [self moveContentviewLeft];
                
                self.mainImageButton.frame = CGRectMake((self->_imagesAndTitle.count * (self.frameWidth + marginWith)), 0, self.frameWidth, self.frameWidth);
                self.frame = CGRectMake(self.frame.origin.x  - self->_imagesAndTitle.count * (self.frameWidth + marginWith), self.frame.origin.y, (WIDTH + self->_imagesAndTitle.count * (self.frameWidth + marginWith)) ,self.frameWidth);
            }
            if (self->_bgcolor) {
        ...
        [UIView animateWithDuration:showDuration animations:^{
            ...
            if (self.frame.origin.x + self.mainImageButton.frame.origin.x <= kScreenWidth/2) {
                ...
                self.frame = CGRectMake(self.frame.origin.x + self->_imagesAndTitle.count * (self.frameWidth + marginWith), self.frame.origin.y, self.frameWidth ,self.frameWidth);
            }
            self.backgroundColor = [UIColor clearColor];
        }];
        [self performSelector:@selector(changeStatus) withObject:nil afterDelay:statusChangeDuration];
...
- (void)changeStatus
{
    [UIView animateWithDuration:1.0 animations:^{
        self->_mainImageButton.alpha = sleepAlpha;
    }];
